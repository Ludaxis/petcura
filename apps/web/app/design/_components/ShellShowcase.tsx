...
import { Button, Eyebrow, SegmentedControl, cn } from "@petcura/ui";
import {
  Dialog,
  DialogBody,
  DialogContent,
...
// ---------------------------------------------------------------------------
// F. New primitives showcase — Dialog, SegmentedControl, Eyebrow
// ---------------------------------------------------------------------------

/**
 * Live demos of the three primitives extracted by claude/ui-ds-cleanup so a
 * future PR can't regress them without one of these tiles going dark first.
 * Each demo mirrors a real call-site shape so the visual contract stays
 * honest:
 *   - Dialog → command-palette / shortcut-sheet / edit-modal shells.
 *   - SegmentedControl → inbox view toggle / density / theme switcher.
 *   - Eyebrow → AI draft eyebrow / side-block caption / shortcut hint.
 */

export function DialogPrimitiveShowcase() {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-[var(--line)] bg-[var(--paper)] p-6">
      ...
      <p className="text-[12.5px] leading-[1.5] text-[var(--ink-2)]">
        The shared <code className="font-mono text-[11.5px]">Dialog</code>{" "}
        primitive replaces five hand-rolled shells (command palette,
        shortcut sheet on /inbox and /requests/[id], AI draft edit modal,
        create-reminder dialog). Sizes: sm = 440px, md = 560px, lg = 720px.
        Scrim, focus trap, Escape, click-outside, and focus restoration are
        owned by the primitive — call sites only flip the open state.
      </p>
      <div className="flex flex-wrap gap-2">
        <Dialog open={smallOpen} onOpenChange={setSmallOpen}>
          ...
          <DialogContent size="sm" aria-label="Reminder · sample dialog">
            <DialogHeader>
              <DialogTitle>Create reminder</DialogTitle>
            </DialogHeader>
            <DialogBody>
              <p className="text-[13px] leading-[1.55] text-[var(--ink-2)]">
                Small dialogs (440px) host short forms — create-reminder is
                the canonical example. The header rule + scrolling body +
                sticky footer compose every shape the app needs.
              </p>
            </DialogBody>
            <DialogFooter>
        ...
        <Dialog open={mediumOpen} onOpenChange={setMediumOpen}>
          ...
          <DialogContent size="md" aria-label="Shortcuts · sample dialog">
            <DialogHeader>
        ...
        <Dialog open={largeOpen} onOpenChange={setLargeOpen}>
          ...
          <DialogContent size="lg" aria-label="Command palette · sample dialog">
            <DialogHeader>
              <DialogTitle>Command palette</DialogTitle>
            </DialogHeader>
            <DialogBody>
              <p className="text-[13px] leading-[1.55] text-[var(--ink-2)]">
                Large dialogs (720px) host the command palette and any
                future list-based overlays. The body scrolls internally so
                the dialog never pushes past the safe area on small
                heights.
              </p>
            </DialogBody>
          </DialogContent>
...
export function SegmentedControlShowcase() {
  ...
  return (
    <div className="flex flex-col gap-5 rounded-xl border border-[var(--line)] bg-[var(--paper)] p-6">
      <Eyebrow tone="muted" size="sm" bold>
        SegmentedControl
      </Eyebrow>
      <p className="text-[12.5px] leading-[1.5] text-[var(--ink-2)]">
        Direction-B radiogroup with roving tabindex + arrow-key navigation.
        Replaces four hand-rolled toggles (inbox view, density, theme).
        Use <code className="font-mono text-[11.5px]">tone=&quot;neutral&quot;</code>{" "}
        for ink-on-paper inversion (toolbar context) and{" "}
        <code className="font-mono text-[11.5px]">tone=&quot;primary&quot;</code>{" "}
        for sage-on-paper (the theme picker inside the user menu).
      </p>
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <span className="w-[120px] text-[11.5px] font-mono uppercase tracking-[0.05em] text-[var(--muted-2)]">
            View · neutral
          </span>
          <SegmentedControl
            value={view}
            onValueChange={(next: "list" | "board") => setView(next)}
            aria-label="View"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <SegmentedControl
            value={density}
            onValueChange={(next: "comfortable" | "compact") =>
              setDensity(next)
            }
            aria-label="Density"
            size="md"
          >
            <SegmentedControl.Item value="comfortable" label="Comfortable" />
            <SegmentedControl.Item value="compact" label="Compact" />
          </SegmentedControl>
        </div>
        <div className="flex items-center gap-3">
          <span className="w-[120px] text-[11.5px] font-mono uppercase tracking-[0.05em] text-[var(--muted-2)]">
          ...
          <SegmentedControl
            value={theme}
            onValueChange={(next: "light" | "dark" | "system") =>
              setTheme(next)
            }
            aria-label="Theme"
            tone="primary"
...
export function EyebrowShowcase() {
  return (
    <div className="flex flex-col gap-5 rounded-xl border border-[var(--line)] bg-[var(--paper)] p-6">
      <Eyebrow tone="muted" size="sm" bold>
        Eyebrow · tones & sizes
      ...
      <p className="text-[12.5px] leading-[1.5] text-[var(--ink-2)]">
        Small uppercase mono caption used above section headers, on AI
        draft cards, on side-block accordions, and on shortcut hints.
        Replaces ~a dozen ad-hoc{" "}
        <code className="font-mono text-[11.5px]">
          font-mono text-[10.5px] uppercase tracking-…
        </code>{" "}
        instances. The primitive is polymorphic via{" "}
        <code className="font-mono text-[11.5px]">as</code> so it can render
        as <code className="font-mono text-[11.5px]">h2</code> inside a{" "}
        <code className="font-mono text-[11.5px]">summary</code>.
      </p>
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
        <div className="flex flex-col gap-1">
          <Eyebrow tone="muted">muted · md</Eyebrow>
          <span className="text-[11px] text-[var(--muted-2)]">default</span>
        </div>
        <div className="flex flex-col gap-1">
          <Eyebrow tone="muted-2">muted-2 · md</Eyebrow>
          <span className="text-[11px] text-[var(--muted-2)]">disabled</span>
        </div>
        <div className="flex flex-col gap-1">
          <Eyebrow tone="primary" bold>
            primary · md · bold
          </Eyebrow>
        ...
        <div className="flex flex-col gap-1">
          <Eyebrow tone="amber" bold>
            amber · md · bold
          </Eyebrow>
          <span className="text-[11px] text-[var(--muted-2)]">warning</span>
        </div>
        <div className="flex flex-col gap-1">
          <Eyebrow tone="red" bold>
            red · md · bold
          </Eyebrow>
          <span className="text-[11px] text-[var(--muted-2)]">urgent</span>
        </div>
        <div className="flex flex-col gap-1">
