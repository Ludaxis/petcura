...
export function OtpForm({ locale }: Props) {
  ...
  async function onSubmitPhone(e: FormEvent) {
    ...
    setPending(true);
    // TODO: call requestOwnerOtp(phone) per owner-auth.md
  ...
  async function onSubmitOtp(e: FormEvent) {
    ...
    setPending(true);
    // TODO: call verifyOwnerOtp(phone, code); on success redirect to /o
      ...
      <form onSubmit={onSubmitOtp} className="flex flex-col gap-5">
        ...
        <button
          ...
          className={cn(
            ...
            "hover:text-[var(--ink)] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)] focus-visible:ring-offset-2"
          )}
        >
          {t("login.otp.resend")}
        </button>
