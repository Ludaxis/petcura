...
export async function getRequestDetail(
  ...
  const { data: requestData, error: requestError } = await supabase
    .from("requests")
    .select(
      "id, assigned_staff_id, owner_id, pet_id, category, channel, status, urgency, ai_summary, ai_summary_version, urgency_suggestion, risk_flags_json, created_at, updated_at, owners(id, name, phone, preferred_language), pets(id, name, species, breed, photo_url)"
    )
    .eq("clinic_id", clinicId)
    .eq("id", requestId)
    .maybeSingle();
